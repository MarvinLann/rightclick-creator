...
- 打开合伙人改过的合同，**修订痕迹、批注、混乱字体**混在一起，得花十分钟手动接受修订、统一格式
...
| 工具 | 场景 | 效果 |
|------|------|------|
| **word整理** | 合伙人改过的合同/起诉状，修订痕迹+批注+混乱字体 | 一键接受所有修订、删除批注、统一格式 |
...
对方律师发来 PDF 答辩状，你要在自己的起诉状里引用原文。
**原来：** 手动复制粘贴 + 调格式 + 排版，20 分钟。
...
**原来：** 打开 Word → 全选接受修订 → 删批注 → 调格式 → 另存 PDF，15 分钟。
**现在：** 两次右键，干净 PDF 直接出来。
...
```
Excel格式整理  →  Word表格横排（对Excel无用，但演示工具组合思路）
```

助理用 Excel 做了证据清单，格式混乱，且表格太宽。
**原来：** 逐行调格式 → 截图贴 Word → 调横向 → 打印。
**现在：** Excel格式整理一次搞定格式，再用 Word表格横排处理跨页问题。
...
|------|-------------|-------------|
| 生成信息图 | DeepSeek 或 通义千问 | DeepSeek、通义千问 |
| MD整理 | 通义千问 | 通义千问 |
